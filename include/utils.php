...
function folder_info($path,$count_hidden=true)
{
	...
	foreach ($files as $t)
	{
		if ($t != "." && $t != "..")
		{
			if ($count_hidden or !(in_array($t, is_array($config['hidden_folders']) ? $config['hidden_folders'] : array()) or in_array($t, is_array($config['hidden_files']) ? $config['hidden_files'] : array())))
			{
